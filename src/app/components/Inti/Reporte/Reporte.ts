...
export class Reporte {
    ...
    // Materia equivale a asignatura (supongo)
    ...
    // Para abriri las ventanas modales (supongo)
    openSM(contenido: any) {
        this.modal.open(contenido, { size: 'lg', centered: true });
    }

    // Setters, para poder enviar los datos a las cookies
    setAsignatura(nombreAsignatura: string): void {
        this.materiaSeleccionada = nombreAsignatura;
    }
    ...
    setSO(so: string): void {
        this.soSeleccionado = so;
    }

    setPI(pi: string): void {
        this.piSeleccionado = pi;
    }
    ...
    generarReporte() {
        let datosReporte = {
            ...
            semestre: this.periodoSeleccionado,
            so: this.soSeleccionado,
            pi: this.piSeleccionado
        }

        this.router.navigate(['/reporte-generado'])
    }
}